...
public class AudioHandler extends AudioEventAdapter implements AudioSendHandler 
{
    ...
    public int addTrackToFront(QueuedTrack qtrack)
    {
        if(audioPlayer.getPlayingTrack()==null)
        {
            ...
            audioPlayer.setVolume(calculateNormalized(qtrack.getTrack(), 50));
            return -1;
        }
        else
        {
    ...
    public boolean playFromDefault()
    {
        if(!defaultQueue.isEmpty())
        {
            ...
            audioPlayer.setVolume(calculateNormalized(defaultQueue.remove(0), 50));
            return true;
        }
        ...
        pl.loadTracks(manager, (at) -> 
        {
            if(audioPlayer.getPlayingTrack()==null)
            	...
            	audioPlayer.setVolume(calculateNormalized(at, 50));
                }
            else
                defaultQueue.add(at);
        }, () -> 
    ...
    public void onTrackEnd(AudioPlayer player, AudioTrack track, AudioTrackEndReason endReason) 
    {
        ...
        else
        {
            ...
            player.setVolume(calculateNormalized(qt.getTrack(), 50));
        }
    }
    ...
    private int calculateNormalized(AudioTrack audioTrack, int targetVolume)
    {
        //TextChannel abc = manager.getBot().getJDA().getTextChannelById(681708194099494960L);
        ...
        try
        {
            String videoApiUrl = "https://youtube.googleapis.com/v/" + id;
            String encodedApiUrl = URLEncoder.encode(videoApiUrl, UTF_8.name());
            String link = "https://www.youtube.com/get_video_info?video_id=" + id + "&eurl=" + encodedApiUrl +
            "hl=en_GB";

            //abc.sendMessage("link: " + link).complete();
            url = new URL(link);
            ...
            //abc.sendMessage(id).complete();

            Pattern pat = Pattern.compile("loudnessDb%22%3A(.*?)%2C");
            ...
            {
            	scanner.close();
            	throw new Exception("No Loudness");
            }
            ...
            /*
            abc.sendMessage("LoudnessDb: " + mat.group(1)).complete();
            abc.sendMessage("targetVolume: " + targetVolume).complete();
            abc.sendMessage("loudness: " + loudness).complete();
            abc.sendMessage("percent: " + percent).complete();
            abc.sendMessage("normalizedVolume: " + normalizedVolume).complete();
            */
        ...
        {
            normalizedVolume = targetVolume;
            //abc.sendMessage("Couldnt find loudness").complete();
            scanner.close();
            return normalizedVolume;
        }
    }
