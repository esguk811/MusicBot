...
public class SkipCmd extends MusicCommand 
{
    ...
    public void doCommand(CommandEvent event) 
    {
        ...
        {
            event.reply(event.getClient().getSuccess()+" **"+handler.getPlayer().getPlayingTrack().getInfo().title+"** 넘김");
        ...
        else
        {
            ...
            if(handler.getVotes().contains(event.getAuthor().getId()))
                msg = event.getClient().getWarning()+" 이미 넘기기 투표함 `[";
            else
            {
                msg = event.getClient().getSuccess()+" 넘기기 투표함 `[";
                handler.getVotes().add(event.getAuthor().getId());
            }
            int skippers = (int)event.getSelfMember().getVoiceState().getChannel().getMembers().stream()
                    .filter(m -> handler.getVotes().contains(m.getUser().getId())).count();
            int required = (int)Math.ceil(listeners * .55);
            msg+= skippers+"표, "+listeners+"명 중 "+required+"명의 투표 필요]`";
